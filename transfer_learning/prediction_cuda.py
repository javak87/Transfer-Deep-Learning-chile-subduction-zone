...
class Data_Augmentation(object):
    ...
    def __call__ (self, p_th=0.1, s_th=0.6):
        ...
        for stream in self.waveforme_loader (DF_selected_chile_path_file, DF_auxiliary_path_file):
            ...
            numpy_array = self.waveforme_to_numpy (stream)
            cuda_array = cuda.to_device(numpy_array)
            b=1
            '''
    ...
    def waveforme_to_numpy (self, stream):
        ...
        numpy_array = np.concatenate((stream[0].data.reshape(1, stream[0].data.shape[0]),stream[1].data.reshape(1, stream[1].data.shape[0]),stream[2].data.reshape(1, stream[2].data.shape[0])), axis=0)
        return numpy_array
